...
```
docker run -e RUNNER_URL -e RUNNER_KEY -e RUNNER_COURSE_ID -e RUNNER_FOLDER_ID troeger/moodlerunner:0.1.22
```
...
```
docker run --mount type=bind,src=$(PWD),dst=/hostdir troeger/moodlerunner:0.1.22 test helloworld_java/validator.py helloworld_java/working/HelloWorld.java
```
